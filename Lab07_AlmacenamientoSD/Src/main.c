...
int main(void)
{
  ...
  if(fres == FR_OK){
	  transmit_uart("File opened for readind.\n");
  } else if(fres != FR_OK){
	  transmit_uart("File was no opened for reading!\n");
  }
  ...
  while (1)
  {
    /* USER CODE END WHILE */
	  ...
	  case '2':
		  transmit_uart("\r\n");
		  handle_sd_card_operations("MarioSaludando.txt");
		  break;
	  case '3':
		  transmit_uart("\r\n");
		  handle_sd_card_operations("Luigi.txt");
		  break;
	  default:
		  transmit_uart("Opción no válida.\r\n");
		  break;
	  }
